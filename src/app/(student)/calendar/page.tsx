...
import { getNoteCounts } from "@/lib/period-schedule";
...
export default async function CalendarPage({
  ...
  const noteCounts = await getNoteCounts(supabase, firstDay, lastDay);
  ...
  return (
    <div>
      ...
      <div className="px-5 pb-6">
        ...
        <div className="mb-3 flex items-center justify-between">
          <Link href="/timetable" className="text-xs font-semibold text-[var(--color-brand)]">
            내 시간표 보기/수정 →
          </Link>
          <DatePickerNav basePath="/calendar" currentDate={today} />
        </div>
        ...
        <MonthEventList
          days={Array.from({ length: daysInMonth }).map((_, i) => {
            ...
            return {
              ...
              noteCount: noteCounts.get(dateStr) ?? 0,
            };
          })}
          today={today}
        />
